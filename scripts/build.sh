...
zip -r "$OUTPUT_DIR/win-x64.zip" "$OUTPUT_DIR/win-x64"
...
zip -r "$OUTPUT_DIR/win-arm64.zip" "$OUTPUT_DIR/win-arm64"
rm -rf "$OUTPUT_DIR/win-arm64"

echo "Building for Linux x64..."
dotnet publish "$PROJECT_PATH" --runtime linux-x64 --self-contained true --configuration Release --output "$OUTPUT_DIR/linux-x64"
zip -r "$OUTPUT_DIR/linux-x64.zip" "$OUTPUT_DIR/linux-x64"
...
zip -r "$OUTPUT_DIR/linux-arm64.zip" "$OUTPUT_DIR/linux-arm64"
rm -rf "$OUTPUT_DIR/linux-arm64"

echo "Building for macOS x64..."
dotnet publish "$PROJECT_PATH" --runtime osx-x64 --self-contained true --configuration Release --output "$OUTPUT_DIR/osx-x64"
zip -r "$OUTPUT_DIR/osx-x64.zip" "$OUTPUT_DIR/osx-x64"
...
zip -r "$OUTPUT_DIR/osx-arm64.zip" "$OUTPUT_DIR/osx-arm64"
